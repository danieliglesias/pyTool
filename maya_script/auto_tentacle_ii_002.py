...
for x in range(1,5):
    for side in ('l','r'):
        ...
        cmds.skinCluster(ik_list, tentacle_curve, tsb=True , name = '{}_{}_tentacle{}_skincluster'.format(name,side,x))            
        ik_handle = cmds.ikHandle(n='{}_{}_tentacle{}_ikhandle'.format(name, side, x),
                                  ...
                                  curve=tentacle_curve, solver='ikSplineSolver', createCurve=False, parentCurve=False,
                                  numSpans=3)[0]
        ...
        for i in range(1,(midpos+2)):
            animBlendNodeAdditiveRotation = cmds.createNode('animBlendNodeAdditiveRotation',
                                                            n='{}_{}_tentacle{}_animBlendNodeAdditiveRotation{:02d}'.format(name,side, x,(i)))
            cmds.connectAttr('{}.output.outputX'.format(animBlendNodeAdditiveRotation),
                             '{}_{}_tentacle{}_ribbon_{:02d}_joint_def_ctrl_off.rotate.rotateX'.format(name, side,x,(i)),
                             force=True)

        ik_list = [('01_sta'),
                   # ('{:02d}_sub'.format(int(int(midpos) / 2) + 1)),
                   ('{:02d}_mid'.format(int(midpos/2)+1))]
                   # ('{:02d}_bet'.format(int(int(midpos) + ((int(lastpos) - int(midpos)) / 2)) - 1)),
                   #('{:02d}_end'.format(int(lastpos/2)+1))]
        for item in (ik_list):
            if item[3:] == 'sta':
                for i in range(1, (int(int(midpos+1) / 2))):
                    # if (i + 1) % 2 != 0:
                    exist = cmds.attributeQuery('sta_{:02d}'.format((i)),
                                                node='{}_tentacle_rotation_guide'.format(name), ex=True)
                    if not exist:
                        cmds.addAttr(tentacle_guide, longName='sta_{:02d}'.format((i)), defaultValue=0, minValue=-1,
                                     maxValue=1,
                                     keyable=1)
                    multiplyDivide = cmds.createNode('multiplyDivide',
                    ...
                    cmds.connectAttr('{}.outputX'.format(multiplyDivide),
                                     '{}_{}_tentacle{}_animBlendNodeAdditiveRotation{:02d}.inputAX'.format(name, side,
                                                                                                           x, (i)))
                    cmds.connectAttr('{}_tentacle_rotation_guide.sta_{:02d}'.format(name, (i)),
            ...
            if item[3:] == 'mid':
                ...
                for i in range((int(int(midpos+1) / 2)), (int(lastpos/2)+1)): 
                    exist = cmds.attributeQuery('mid_{:02d}'.format((i)),
                                                node='{}_tentacle_rotation_guide'.format(name), ex=True)
                    if not exist:
                        cmds.addAttr(tentacle_guide, longName='mid_{:02d}'.format((i)), defaultValue=0,
                                     minValue=-1,
                                     maxValue=1,
                                     keyable=1)
                    multiplyDivide = cmds.createNode('multiplyDivide',
                    ...
                    cmds.connectAttr('{}.outputX'.format(multiplyDivide),
                                     '{}_{}_tentacle{}_animBlendNodeAdditiveRotation{:02d}.inputAX'.format(name,
                                                                                                           side, x,
                                                                                                           (i)))
                    cmds.connectAttr('{}_tentacle_rotation_guide.mid_{:02d}'.format(name, (i)),
                                     '{}.input2X'.format(multiplyDivide))
...
for x in range(1, 5):
    for side in ('l', 'r'):
        ...
        cmds.addAttr('{}_{}_tentacle{}_27_end_ctrl'.format(name,side,x), longName='FreeArmControls', defaultValue=0,
                                     minValue=0,
                                     maxValue=1,
                                     keyable=1)
        ...
        utili.visibilitySwitch(objectList = listaControles, targetCtrl = '{}_{}_tentacle{}_27_end_ctrl'.format(name,side,x),
